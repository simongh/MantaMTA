...
namespace WebInterface.Controllers
{
    public class SendsController : Controller
    {
		...
		{
			int bounceCount = TransactionDB.GetDeferredCount(sendID);
			int pageCount = (int)Math.Ceiling(bounceCount / Convert.ToDouble(pageSize));
			return View(new SendReportBounces(TransactionDB.GetDeferralInfo(sendID, page, pageSize), sendID, page, pageCount));
		}
		...
		{
			return View(new SendReportSpeed(TransactionDB.GetSendSpeedInfo(sendID), sendID));
		}
